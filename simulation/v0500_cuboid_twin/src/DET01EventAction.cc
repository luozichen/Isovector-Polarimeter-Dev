...
// Number of detectors in the geometry (2 rings × 4 per ring)
static const G4int kNDet = 8;
...
void DET01EventAction::EndOfEventAction(const G4Event* event)
{
  ...
  // Placeholder: Approximate optical jitter to be subtracted from physical total
  // To get the true intrinsic PMT noise. We will tune this later.
  G4double optical_jitter_est = 0.3*ns; 

  // Fix time if no hits, otherwise apply jitter
  for (G4int d = 0; d < kNDet; d++) {
      if (pe[d] == 0) {
          hitTime[d] = -1.0;
      } else {
          // Det 0/4: Left, Det 1/5: Up, Det 2/6: Right, Det 3/7: Down
          G4int phys_idx = d % 4; 
          
          // sigma_elec^2 = sigma_total^2 - sigma_opt^2
          G4double sig_tot = total_jitter[phys_idx];
          G4double sig_elec_sq = (sig_tot*sig_tot) - (optical_jitter_est*optical_jitter_est);
          if(sig_elec_sq < 0) sig_elec_sq = 0; // safety
          G4double sig_elec = std::sqrt(sig_elec_sq);
          
          // Inject electronic noise
          hitTime[d] += G4RandGauss::shoot(0.0, sig_elec);
      }
  }
