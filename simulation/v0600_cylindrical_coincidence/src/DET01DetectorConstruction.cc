...
G4VPhysicalVolume* DET01DetectorConstruction::Construct()
{
  ...
  // --- Two Face-to-Face Cylindrical Detectors ---
  G4double scinDiam = 40.0*mm;
  G4double scinRad = scinDiam/2.0;
  G4double scinThick = 10.0*mm;
  ...
  // Rotations are default since cylinders naturally align along Z
  G4RotationMatrix* rot = new G4RotationMatrix();

  for(G4int i=0; i<nDetectors; i++) {
      G4int copyNo = i;
      
      // i=0 is Top (+Z), i=1 is Bottom (-Z)
      G4double sign = (i == 0) ? 1.0 : -1.0; 
      G4double posZ = sign * (gap/2.0 + scinThick/2.0);
