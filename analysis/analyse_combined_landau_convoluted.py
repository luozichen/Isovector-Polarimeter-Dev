...
# Physical dimensions: Scintillator = 120 x 150 x 150 mm, Gap = 12.5 mm
size_x = 120.0
size_y = 150.0
size_z = 150.0
gap = 12.5
...
def fit_and_plot(ax, data, label, color):
    ...
    if len(data) > 0 and np.max(y) > 0:
        ...
        # Initial guesses widened for convoluted shape
        p0 = [mpv_guess, 0.01, np.max(y)*1.0] 
        
        try:
            popt, _ = curve_fit(convoluted_landau_fit_func, bin_centers, y, 
                                p0=p0, 
                                bounds=([0, 0, 0], [1, 1, np.inf]))
            
            x_fine = np.linspace(LANDAU_RANGE[0], LANDAU_RANGE[1], 200)
            ax.plot(x_fine, convoluted_landau_fit_func(x_fine, *popt), 'k--', lw=1.5)
...
def main():
    ...
    fig.suptitle("Convoluted Landau Fits: Individual Middle Runs vs Combined (Physical Collimation)", fontsize=16)
    ...
    out_path = os.path.join(RESULTS_DIR, "combined_landau_grid_convoluted.png")
